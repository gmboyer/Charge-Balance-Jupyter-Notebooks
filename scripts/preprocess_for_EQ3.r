...
# function to print messages if verbose == TRUE
...
# function to get units from a header
...
# function to get chemical species from header
...
num_cols <- colnames(select(df, -one_of(exclude)))
    ...
    if(length(redox_col_index) > 0){
      if(!is.na(df[row, redox_col_index])){
        ...
        vprint(paste0("Warning: non-numeric pe value in sample ",
                       df[row, "Sample"], ". Resorting to using ",
                       "Log fO2 (log bars) with a value of ", default_logfO2), verbose=verbose)
        this_redox_flag <- 0
        this_redox_value <- sprintf("%.4E", default_logfO2)
        this_redox_unit <- paste0("Log fO2 (log bars) [default = ", default_logfO2, "]")
        assigned <- TRUE
    ...
    if(length(redox_col_index) > 0){
      ...
      if(!warned_about_redox_column){
        vprint(paste0("Warning: no 'Eh' column found with 'volts' units. ",
                      ...
                      default_logfO2), verbose=verbose)
        warned_about_redox_column <- TRUE
      }
      this_redox_flag <- 0
      this_redox_value <- sprintf("%.4E", default_logfO2)
    ...
    if(length(redox_col_index) > 0){
      if(!is.na(df[row, redox_col_index])){
        ...
        vprint(paste0("Warning: non-numeric logfO2 value in sample ",
                       df[row, "Sample"], ". Resorting to using ",
                       "a logfO2 value of ", default_logfO2), verbose=verbose)
        this_redox_flag <- 0
        this_redox_value <- sprintf("%.4E", default_logfO2)
        this_redox_unit <- paste0("Log fO2 (log bars) [default = ", default_logfO2, "]")
        assigned <- TRUE
      ...
      if(!warned_about_redox_column){
        vprint(paste("Warning: no 'logfO2' column found. Attempting to find a",
                    "'O2,AQ' column to estimate logfO2 at sample temperature and",
                    "pressure..."), verbose=verbose)
        warned_about_redox_column <- TRUE
      }
      redox_col_index <- grep("^O2,AQ", names(df))
      if(length(redox_col_index) > 0){
        if(!is.na(df[row, redox_col_index])){
          ...
          if(this_header_unit %in% acceptable_units){
            ...
            vprint(paste("Warning: O2,AQ column found, but units are not ppb, ppm",
          ...
          vprint(paste0("Warning: non-numeric O2,AQ value in sample ",
                         df[row, "Sample"], ". Resorting to using ",
                         "Log fO2 (log bars) with a value of ", default_logfO2),
                         verbose=verbose)
          this_redox_flag <- 0
          ...
          if(!warned_about_redox_column){
            vprint(paste("Warning: 'O2,AQ' column not found. Resorting to",
...
vprint(paste("Writing csv summary called", csv_filename, "..."), verbose=verbose)
write.csv(df, csv_filename, row.names=F)

this_time <- proc.time() - ptm                             

print(paste("Done! Took", round(this_time["elapsed"], 1), "seconds."))
